...
int main()
{
    ...
    for (int i = OrderNum; i >= 1; i--)
    {
        for (int j = 1; j <= i - 1; j++)
        {
            if ((deadLine[j] > deadLine[j + 1]) || ((deadLine[j] == deadLine[j + 1]) && (orderArray[j] > orderArray[j + 1])) || ((deadLine[j] == deadLine[j + 1]) && (orderArray[j] == orderArray[j + 1]) && (orderNumber[j] == orderNumber[j + 1])))
            {
                int tmp1 = deadLine[j];
                deadLine[j] = deadLine[j + 1];
                deadLine[j + 1] = tmp1;

                int tmp2 = orderArray[j];
                orderArray[j] = orderArray[j + 1];
                orderArray[j + 1] = tmp2;

                int tmp3 = orderNumber[j];
                orderNumber[j] = orderNumber[j + 1];
                orderNumber[j + 1] = tmp3;
            }
