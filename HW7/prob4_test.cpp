...
int main()
{
    ...
    for (int i = locationNum; i >= 1; i--)
    {
        for (int j = 1; j <= i - 1; j++)
        {
            if (airportName[j][0] > airportName[j + 1][0] || airportName[j][1] > airportName[j + 1][1] || airportName[j][2] > airportName[j + 1][2])
            {
                char* tmp = airportName[j];
                airportName[j] = airportName[j + 1];
                airportName[j + 1] = tmp;

                int tempCost = costArray[j];
                costArray[j] = costArray[j + 1];
                costArray[j + 1] = tempCost;
            }
